...
- **Switching is not generic failover:** the current multi-configuration scheduler switches when Pi reports a content-audit failure or an HTTP 502 status error (both surface as `PiAuditError`), not on every timeout, login error, or provider failure. After 600 seconds without another switch-triggering event, a subsequent request can restore the first-priority entry. All configured agents must be installed and usable before starting; fallback is not a substitute for setup.
