...
from .llm.providers._cli import terminate_all_active_processes
from .llm.status import (
    ERROR as REQUEST_ERROR,
)
from .llm.status import (
    REQUESTING,
...
from .llm.status import (
    SUCCESS as REQUEST_SUCCESS,
)
from .llm.status import (
    TIMEOUT as REQUEST_TIMEOUT,
)
...
def _handle_interrupt_signal(signum: int, frame: Any) -> None:
    """响应 SIGINT (Ctrl+C) / SIGBREAK，清理子进程并快速安全退出。"""
    global _INTERRUPTED
...
def install_signal_handlers() -> None:
    """安装 CLI 的 SIGINT 与 SIGBREAK 信号处理器。"""
    if os.environ.get("TRANS_NOVEL_DISABLE_SIGNAL_HANDLERS") == "1":
        return
    if "pytest" in sys.modules and os.environ.get("TRANS_NOVEL_FORCE_SIGNAL_HANDLERS") != "1":
        return
    try:
...
_STATUS_STYLES = {
    ...
    REQUEST_TIMEOUT: "magenta",
    REQUEST_ERROR: "red",
}


class RequestStatusColumn(ProgressColumn):
    """在 Rich 进度条中显示当前请求和上一次请求结果。"""

    def __init__(self, tracker: RequestStatusTracker) -> None:
        super().__init__()
    ...
    def render(self, task) -> Text:
        ...
        cfg_label = f"LLM 配置：{snapshot.config_index}号"
        if current is None:
            return Text(f"{cfg_label} · 请求：尚未开始", style="dim")

        current_label = _STATUS_LABELS[current.status]
        current_style = _STATUS_STYLES[current.status]
        parts = [
            (f"{cfg_label} · 当前请求：", "bold"),
            (current_label, current_style),
        ]
        if snapshot.active_count > 1:
            parts.extend(
                [
                    (f" ({snapshot.active_count} 个并发)", "dim"),
                ]
            )
        if current.stage:
            parts.append((f" · {current.stage}", "dim"))
        if current.elapsed > 0:
            parts.append((f" {current.elapsed:.1f}s", "dim"))
        if current.failed:
            if current.error_type:
                parts.append((f" · {current.error_type}", "red"))
            if current.error_message:
                parts.append((f" · {current.error_message}", "red"))
        ...
        if previous is not None:
            parts.extend(
                [
                    (" ｜ 上一次请求：", "bold"),
                    (_STATUS_LABELS[previous.status], _STATUS_STYLES[previous.status]),
                ]
            )
            if previous.stage:
                parts.append((f" · {previous.stage}", "dim"))
            if previous.failed:
                if previous.error_type:
                    parts.append((f" · {previous.error_type}", "red"))
                if previous.error_message:
                    parts.append((f" · {previous.error_message}", "red"))
...
def _attach_request_status(client: Any, tracker: RequestStatusTracker) -> None:
    """给真实 LLM client 安装 CLI 状态监听器；测试替身可不实现该接口。"""
    config_index = getattr(client, "current_config_index", getattr(client, "config_index", None))
    if config_index is not None and isinstance(config_index, int):
        tracker.set_config_index(config_index)
    setter = getattr(client, "set_status_listener", None)
    if callable(setter):
        setter(tracker)


def _detach_request_status(client: Any, tracker: RequestStatusTracker) -> None:
    """解除 CLI listener，避免长生命周期调用泄漏 Rich tracker。"""
    setter = getattr(client, "set_status_listener", None)
    if callable(setter):
        setter(None)


def _progress_columns(tracker: RequestStatusTracker) -> tuple[ProgressColumn, ...]:
    """返回所有命令共用的 Rich 列定义。"""
    return (
        SpinnerColumn(),
        ...
        MofNCompleteColumn(),
        TimeElapsedColumn(),
        RequestStatusColumn(tracker),
    )


class _ProgressContext:
    """集中管理 Rich Progress 与 LLM 状态 listener 的生命周期。"""

    def __init__(self, client: Any) -> None:
        self.tracker = RequestStatusTracker()
        self.progress = Progress(*_progress_columns(self.tracker), console=console)
        self.client = client

    def __enter__(self) -> Progress:
        _attach_request_status(self.client, self.tracker)
    ...
    def __exit__(self, exc_type, exc, tb) -> bool:
        try:
            ...
            _detach_request_status(self.client, self.tracker)


def _progress_context(client: Any) -> _ProgressContext:
    """为真实编排器或测试替身取得可监听的 client。"""
    return _ProgressContext(getattr(client, "client", client))
...
def _tiers_for_command(command: str | None, config: Config) -> list[str] | None:
    """按子命令返回需要校验凭据的 tier 列表；未指定时校验全量。"""
    if command in ("translate", "prepare"):
        tiers = ["strong", "cheap", "fast"]
        if config.pipeline.review:
            tiers.append(config.pipeline.review_agent_tier)
        return list(dict.fromkeys(tiers))
    if command == "review":
        tiers = ["cheap", config.pipeline.review_agent_tier]
        return list(dict.fromkeys(tiers))
    return None
...
def _validate_api_configuration() -> None:
    ...
    tiers = _tiers_for_command(command, config)
    build_client(config).validate_credentials(tiers)
...
def _translate_srt_or_raise(
    ...
    *,
    ...
    with _progress_context(client) as prog:
...
def _translate_impl_or_raise(
    ...
    *,
    ...
    with _progress_context(orch) as prog:
