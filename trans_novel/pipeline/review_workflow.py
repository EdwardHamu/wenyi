...
from .executor import InterruptibleThreadPoolExecutor
...
class ReviewService:
        ...
        *,
        ...
        if conflict_groups and self._runtime.config.pipeline.review_conflict_arbitration:
            ...
            if workers == 1:
                ...
                with InterruptibleThreadPoolExecutor(max_workers=workers) as executor:
                    futures = {
        ...
        *,
        ...
        try:
            if workers == 1:
                ...
                with InterruptibleThreadPoolExecutor(max_workers=workers) as ex:
                    futures = {
    ...
    def pack_contiguous(segs, budget: int) -> list[list]:
        """按源文 token 预算（tiktoken cl100k_base）把段保序打包成若干连续块。"""
        chunks: list[list] = []
        cur: list = []
        size = 0
        for s in segs:
