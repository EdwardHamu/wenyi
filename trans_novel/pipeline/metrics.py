...
def _serialize_llm_config(llm: LLMConfig) -> dict[str, Any]:
    """把单个 LLMConfig 脱敏后序列化为指纹摘要。"""
    base_url = llm.base_url
    endpoint_identity = _safe_base_url(base_url, include_path=True)
    return {
        "provider": llm.provider,
        "base_url": _safe_base_url(base_url),
        "base_url_fingerprint": (_fingerprint(endpoint_identity) if endpoint_identity else None),
        "reasoning_style": llm.reasoning_style,
        "timeout": llm.timeout,
        "max_retries": llm.max_retries,
        "tiers": {
            name: {
                "provider": tier.provider or llm.provider,
                "model": tier.model,
                "base_url": _safe_base_url(tier.base_url or llm.base_url),
                "base_url_fingerprint": (
                    _fingerprint(
                        _safe_base_url(
                            tier.base_url or llm.base_url,
                            include_path=True,
                        )
                    )
                    if (tier.base_url or llm.base_url)
                    else None
                ),
                "key_env": tier.api_key_env or llm.api_key_env,
                "cli_path": tier.cli_path or llm.cli_path,
                "reasoning_style": (tier.reasoning_style or llm.reasoning_style),
                "options": _safe_value(tier.options),
            }
            for name, tier in sorted(llm.tiers.items())
        },
    }
...
def config_identity(config: Config) -> dict[str, Any]:
    ...
    summary = {
        ...
        "llm_priority": config.llm_priority,
        "llm_list": [_serialize_llm_config(cfg) for cfg in config.llm_list],
        "llm": _serialize_llm_config(config.llm),
