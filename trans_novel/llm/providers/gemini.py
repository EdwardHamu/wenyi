...
class GeminiClient(LLMClient):
    ...
    def __init__(self, cfg: LLMConfig, *, require_strong: bool = True) -> None:
        super().__init__()
        self.cfg = cfg
    ...
    def validate_credentials(self, tiers: Sequence[str] | None = None) -> None:
        ...
        tiers_to_check = (
            [self.tiers[t] for t in tiers if t in self.tiers]
            if tiers is not None
            else list(self.tiers.values())
        )
        ...
        for t in tiers_to_check:
            env_name = t.api_key_env or self.cfg.api_key_env
            cache_key = env_name or DEFAULT_API_KEY_ENV
            if cache_key in checked_keys:
                continue
            api_key, target_env = get_api_key_from_env(env_name)
            if not api_key:
                raise RuntimeError(f"未设置环境变量 {target_env}（或 {FALLBACK_API_KEY_ENV}）")
            checked_keys.add(cache_key)

    def _ensure_client(self, tier_config: ResolvedTier[GeminiTierOptions] | None = None) -> Any:
        ...
        try:
            ...
            raise RuntimeError(
                "需要 google-genai SDK：pip install google-genai（或运行 uv add google-genai）"
            ) from error

        api_key_env = (
            tier_config.api_key_env
            if tier_config and tier_config.api_key_env
            else self.cfg.api_key_env
        )
        base_url = (
            tier_config.base_url if tier_config and tier_config.base_url else self.cfg.base_url
        )
        key = (api_key_env, base_url)

        with self._client_lock:
            if key not in self._clients:
                api_key, target_env = get_api_key_from_env(api_key_env)
                if not api_key:
                    raise RuntimeError(f"未设置环境变量 {target_env}（或 {FALLBACK_API_KEY_ENV}）")
                kwargs: dict[str, Any] = {
                    "api_key": api_key,
                ...
                if base_url:
                    kwargs["http_options"].update({"api_option": "REST", "base_url": base_url})
    ...
    def complete_json(
        ...
        *,
        ...
        """请求 Gemini JSON，并保留基类的解析诊断与事件语义。"""
