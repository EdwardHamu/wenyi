...
import openai  # noqa: F401 - 预热第三方依赖，防止 distro 在 mock isfile 时污染
import typer
...
class TestWindowsConsoleEncoding(unittest.TestCase):
    ...
    def test_tiers_for_command_resolution(self):
        from trans_novel.cli import _tiers_for_command
        from trans_novel.config import Config

        cfg = Config.from_dict({"pipeline": {"review": False}})
        self.assertEqual(
            _tiers_for_command("translate", cfg),
            ["strong", "cheap", "fast"],
        )
        self.assertEqual(
            _tiers_for_command("prepare", cfg),
            ["strong", "cheap", "fast"],
        )
        self.assertEqual(
            _tiers_for_command("review", cfg),
            ["cheap", "strong"],
        )
        self.assertIsNone(_tiers_for_command("unknown", cfg))

        # With review enabled and custom review_agent_tier
        cfg_review = Config.from_dict(
            {
                "pipeline": {
                    "review": True,
                    "review_agent_tier": "fast",
                }
            }
        )
        self.assertEqual(
            _tiers_for_command("translate", cfg_review),
            ["strong", "cheap", "fast"],
        )
        self.assertEqual(
            _tiers_for_command("review", cfg_review),
            ["cheap", "fast"],
        )
