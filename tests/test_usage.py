...
class TestRunStoreLock(unittest.TestCase):
    ...
    def test_config_identity_fingerprint_tracks_tier_provider_and_connections(self):
        from trans_novel.config import Config
        from trans_novel.pipeline.metrics import config_identity
        ...
        base_fp = config_identity(base)["fingerprint"]

        # Change tier provider
        c_provider = base.model_copy(deep=True)
        c_provider.llm.tiers["strong"].provider = "pi"
        self.assertNotEqual(base_fp, config_identity(c_provider)["fingerprint"])

        # Change tier base_url
        c_url = base.model_copy(deep=True)
        c_url.llm.tiers["cheap"].base_url = "https://other.deepseek.com/v1"
        self.assertNotEqual(base_fp, config_identity(c_url)["fingerprint"])

        # Change tier api_key_env
        c_key = base.model_copy(deep=True)
        c_key.llm.tiers["cheap"].api_key_env = "KEY_2"
        self.assertNotEqual(base_fp, config_identity(c_key)["fingerprint"])

        # Change tier cli_path
        c_cli = base.model_copy(deep=True)
        c_cli.llm.tiers["strong"].cli_path = r"D:\other\codex.cmd"
        self.assertNotEqual(base_fp, config_identity(c_cli)["fingerprint"])

        # Change tier reasoning_style
        c_style = base.model_copy(deep=True)
        c_style.llm.tiers["cheap"].reasoning_style = "deepseek"
        self.assertNotEqual(base_fp, config_identity(c_style)["fingerprint"])

    def test_config_identity_tracks_llm_priority_and_list(self):
        from trans_novel.config import Config
        from trans_novel.pipeline.metrics import config_identity
        ...
        cfg1 = Config.from_dict(
            {
                ...
                "llm_list": [
                    {
                        "provider": "pi",
                        "cli_path": r"C:\path\to\pi.cmd",
                        "tiers": {"strong": {"model": "m1"}},
                    },
                    {
                        "provider": "agy",
                        "cli_path": r"C:\path\to\agy.cmd",
                        "tiers": {"strong": {"model": "m2"}},
                    },
                ],
            }
        )
        ident1 = config_identity(cfg1)
        fp1 = ident1["fingerprint"]
        summary1 = ident1["summary"]
        self.assertEqual(summary1["llm_priority"], "01")
        self.assertEqual(len(summary1["llm_list"]), 2)

        # Different priority order changes fingerprint
        ...
        ident2 = config_identity(cfg2)
        self.assertEqual(ident2["summary"]["llm_priority"], "10")
        self.assertNotEqual(fp1, ident2["fingerprint"])

    def test_routed_client_records_by_tier_and_by_provider(self):
        from trans_novel.config import Config
        from trans_novel.llm.providers.fake import FakeClient
        ...
        sub_fake.provider_name = "FakeProvider"
        sub_fake.usage.record(
            "strong",
        ...
        self.assertEqual(summary["by_tier"]["strong"]["total_tokens"], 150)
        self.assertEqual(summary["by_provider"]["FakeProvider"]["total_tokens"], 150)
