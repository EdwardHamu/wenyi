...
class TestReviewer(unittest.TestCase):
    ...
    def test_chapter_review_chunks_run_concurrently_and_merge_in_order(self):
        ...
        cfg = _cfg()
        cfg.segment.max_tokens_per_batch = 1  # 审校预算=3，使两个 4-token 段落各成一块
...
class TestPolisher(unittest.TestCase):
    ...
    def test_polish_continue_appends_user_turn_to_translation_transcript(self):
        ...
        turn = [
            {"role": "system", "content": "你是一位资深的文学翻译"},
            {"role": "user", "content": "待译段落"},
            {
                "role": "assistant",
        ...
        self.assertIn("续写润色指令", messages[-1]["content"])
        self.assertEqual(client.calls[-1]["tier"], "strong")
        self.assertEqual(client.calls[-1]["stage"], "Polisher")
