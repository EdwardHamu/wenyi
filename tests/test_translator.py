...
class TestTranslatorAlignment(unittest.TestCase):
    ...
    def test_nonlinguistic_table_cells_are_preserved_without_model_input(self):
        def handler(messages, tier, json_mode):
            ...
            self.assertNotIn("-", user.split("【待译", 1)[-1].split("【下一段原文", 1)[0])
    ...
    def test_rejected_batch_response_is_logged_before_retry(self):
        responses = iter(
            [
                '{"translations":["只有一段"]}',
                '{"translations":["译0","译1"]}',
        ...
        events = []
        client = FakeClient(handler=lambda messages, tier, json_mode: next(responses))
        client.set_event_sink(lambda event, **data: events.append({"event": event, **data}))

        result = Translator(client, self._config()).translate_batch(["あ", "い"])

        self.assertEqual(result, ["译0", "译1"])
        self.assertEqual(len(events), 1)
        event = events[0]
        self.assertEqual(event["event"], "translator_response_rejected")
        self.assertEqual(event["reason"], "translations_count_mismatch")
        self.assertEqual(event["mode"], "batch")
        self.assertEqual(event["attempt"], 1)
        self.assertEqual(event["attempt_limit"], 2)
        self.assertEqual(event["source_indices"], [0, 1])
        self.assertEqual(event["next_action"], "retry_batch")
        self.assertEqual(event["response"], '{"translations":["只有一段"]}')
        self.assertEqual(event["response_chars"], len(event["response"]))

    def test_malformed_batch_response_is_logged_before_retry(self):
        responses = iter(["模型说明文字，不是 JSON", '{"translations":["译文"]}'])
        events = []
        client = FakeClient(handler=lambda messages, tier, json_mode: next(responses))
        client.set_event_sink(lambda event, **data: events.append({"event": event, **data}))

        self.assertEqual(Translator(client, self._config()).translate_batch(["あ"]), ["译文"])

        self.assertEqual(events[0]["reason"], "invalid_json")
        self.assertEqual(events[0]["response"], "模型说明文字，不是 JSON")
        self.assertEqual(events[0]["next_action"], "retry_batch")

    def test_rejected_single_fallback_response_is_logged_before_abort(self):
        client = FakeClient(
            handler=lambda messages, tier, json_mode: json.dumps({"translations": []})
        )
        events = []
        ...
        with self.assertRaisesRegex(Exception, "第 0 段失败"):
            Translator(client, self._config()).translate_batch(["あ", "い"])

        self.assertEqual(events[-1]["mode"], "single_fallback")
        self.assertEqual(events[-1]["source_indices"], [0])
        self.assertEqual(events[-1]["next_action"], "abort_batch")
        self.assertEqual(events[-1]["response"], '{"translations": []}')
