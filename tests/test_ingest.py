...
class TestTextIngest(unittest.TestCase):
    ...
    def test_batching(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            write_sample_txt(p)
            doc = load_document(p, "ja", "zh")
        batches = chapter_batches(doc.chapters[0], max_tokens=60)
        # 总段数守恒
        total = sum(len(b) for b in batches)
        self.assertEqual(total, len(doc.chapters[0].text_segments))
        self.assertGreater(len(batches), 1)  # 60 token 预算应切出多批
...
class TestSplitLongSegments(unittest.TestCase):
    def test_split_by_sentence_and_cont_flag(self):
        long_src = "第一句。" * 10  # cl100k_base 下 50 token
        ch = Chapter(
            index=0,
            title="章",
            segments=[
    ...
    def test_oversized_codepoint_terminates_loop(self):
        chunks = _split_text("hello", 0)
