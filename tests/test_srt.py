...
class TestSrtTranslate(unittest.TestCase):
    ...
    def test_cli_translate_routes_srt(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            passed_client = translate_srt_mock.call_args[1].get("client")
            self.assertIsNotNone(passed_client)
            self.assertEqual(getattr(passed_client, "config_index", 0), 0)

    def test_cli_translate_srt_binds_priority_client(self):
        with tempfile.TemporaryDirectory() as directory:
            path = os.path.join(directory, "demo.srt")
            with open(path, "w", encoding="utf-8") as handle:
                handle.write(_sample_srt())
            ...
            with (
                ...
                load_config.return_value = Config.from_dict(
                    {
                        "llm_priority": "10",
                        "llm_list": [
                            {"provider": "fake"},
            ...
            passed_client = translate_srt_mock.call_args[1].get("client")
            self.assertIsNotNone(passed_client)
            self.assertEqual(passed_client.current_config_index, 1)
